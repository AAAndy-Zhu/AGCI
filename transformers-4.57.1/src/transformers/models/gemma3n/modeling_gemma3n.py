...
class Gemma3nModel(Gemma3nPreTrainedModel):
    ...
    def adaptive_global_context_broadcast(self, image_tokens, lambda_scale=1.0):
        """
        image_tokens: [B, N, D]
        """
        # global context
        global_ctx = image_tokens.mean(dim=1, keepdim=True)  # [B, 1, D]

        # cosine similarity
        sim = F.cosine_similarity(image_tokens, global_ctx, dim=-1)  # [B, N]

        # context deficiency weight
        w = 1.0 - sim
        w = w.unsqueeze(-1)  # [B, N, 1]

        updated_tokens = image_tokens + lambda_scale * w * global_ctx
    ...
    def forward(
        ...
        **lm_kwargs,
        ...
        if pixel_values is not None:
            ...
            # print('------------------------------------')
            # print(f"Gemma3nModel: global_context_broadcast_lambda = {global_context_broadcast_lambda}")
            if global_context_broadcast_lambda > 0.0:
                image_features = self.adaptive_global_context_broadcast(image_features, lambda_scale=global_context_broadcast_lambda)
            # print(image_features)
            # exit()
...
class Gemma3nForConditionalGeneration(Gemma3nPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: Gemma3nConfig):
        ...
        print(f"Gemma3nForConditionalGeneration: global_context_broadcast_lambda = {self.global_context_broadcast_lambda}")
        self.post_init()
